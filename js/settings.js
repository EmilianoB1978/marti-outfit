...
function initMenu() {
  ...
  const ALL_KEYS = ["calendar", "trips", "budget", "notes", "reminders", "diary", "capsules",
  ...
  btnReset.addEventListener("click", () => {
    Theme.set("menuOrder", ["diary", "reminders", "notes", "budget", "trips", "live", "palette", "dormant", "analytics", "capsules", "calendar", "taxonomies", "settings", "manual", "system"]);
...
// =============================================================================
// Tab "🎨 Chip": personalizzazione visuale di colors / colors-secondary /
// patterns / occasions. Per ogni voce della tassonomia mostra:
//   - Anteprima della chip con bg + pattern + emoji
//   - Color picker per bg
//   - Per occasions: emoji input
//   - Per patterns: select del motivo
//   - Bottone reset
// I default vengono dal modulo chip-styles.js. Modifiche salvate in
// Theme.preferences.chipStyles.
// =============================================================================
async function initChipStyles() {
  const root = document.getElementById("chip-editor-root");
  if (!root) return;
  await Taxonomies.load();

  // Tassonomie da editare. patterns ha select speciale, occasions ha emoji.
  const SECTIONS = [
    { key: "colors",           label: "Colori principali" },
    { key: "colors-secondary", label: "Colori secondari", aliasOf: "colors" },
    { key: "patterns",         label: "Pattern / motivi" },
    { key: "occasions",        label: "Occasioni" },
  ];

  function render() {
    root.innerHTML = SECTIONS.map(s => {
      const taxKey = s.aliasOf || s.key;
      const values = Taxonomies.listSimpleValues(taxKey);
      const items = values.map(v => renderRow(s.key, v)).join("");
      return `<div class="chip-editor-section">
        <h3 class="chip-editor-section-title">${escSet(s.label)}</h3>
        <div class="chip-editor-grid">${items}</div>
      </div>`;
    }).join("");
    bindRows();
  }

  function renderRow(taxonomy, value) {
    const style = ChipStyles.getChipStyle(taxonomy, value);
    const css = ChipStyles.styleToCss(style);
    const previewIcon = style?.icon ? `${style.icon} ` : "";
    const bgVal = (style && style.bg) ? style.bg : "#cccccc";
    const showIconInput = (taxonomy === "occasions");
    const showPatternSelect = (taxonomy === "patterns");
    const patternOptions = Object.keys(ChipStyles.PATTERN_BG)
      .map(p => `<option value="${escSet(p)}"${(style?.pattern === p) ? " selected" : ""}>${escSet(p)}</option>`)
      .join("");

    return `<div class="chip-editor-row" data-tax="${escSet(taxonomy)}" data-val="${escSet(value)}">
      <div class="chip-editor-preview-wrap">
        <span class="chip-editor-preview" style="${css}">${previewIcon}${escSet(capWord(value))}</span>
      </div>
      <div class="chip-editor-label">${escSet(value)}</div>
      <div class="chip-editor-controls">
        <input type="color" class="chip-editor-color-input" value="${toHex(bgVal)}" data-field="bg" aria-label="Colore" />
        ${showIconInput ? `<input type="text" class="chip-editor-icon-input" value="${escSet(style?.icon || "")}" maxlength="2" placeholder="🌟" data-field="icon" aria-label="Emoji" />` : ""}
        ${showPatternSelect ? `<select class="chip-editor-pattern-select" data-field="pattern">${patternOptions}</select>` : ""}
        <button type="button" class="chip-editor-reset-btn" data-action="reset" aria-label="Ripristina">↺</button>
      </div>
    </div>`;
  }

  function bindRows() {
    root.querySelectorAll(".chip-editor-row").forEach(row => {
      ...
      row.querySelectorAll("[data-field]").forEach(input => {
        input.addEventListener("input", () => {
          ...
          if (field === "bg") {
            patch.bg = value;
            patch.fg = ChipStyles.bestTextFor(value);
          } else if (field === "icon") {
            patch.icon = value.trim();
          } else if (field === "pattern") {
            patch.pattern = value;
          }
          ChipStyles.setChipStyle(tax, val, patch);
          updateRowPreview(row, tax, val);
        });
      });

      row.querySelector('[data-action="reset"]').addEventListener("click", () => {
        ChipStyles.resetChipStyle(tax, val);
        // Re-render della singola row
        const next = renderRow(tax, val);
        const tmp = document.createElement("div");
        tmp.innerHTML = next;
        const newRow = tmp.firstElementChild;
        row.replaceWith(newRow);
        // Re-bind sulla nuova row
        bindSingleRow(newRow);
      });
    });
  }
  ...
  function updateRowPreview(row, tax, val) {
    const style = ChipStyles.getChipStyle(tax, val);
    const css = ChipStyles.styleToCss(style);
    const previewIcon = style?.icon ? `${style.icon} ` : "";
    const preview = row.querySelector(".chip-editor-preview");
    if (!preview) return;
    preview.setAttribute("style", css);
    preview.innerHTML = `${previewIcon}${escSet(capWord(val))}`;
  }

  function toHex(c) {
    if (typeof c !== "string") return "#cccccc";
    if (c.startsWith("#") && (c.length === 7 || c.length === 4)) {
      // espandi #abc -> #aabbcc per il color picker
      if (c.length === 4) {
  ...
  // Reset globale
  const resetBtn = document.getElementById("btn-reset-chips");
  if (resetBtn) {
    resetBtn.addEventListener("click", () => {
      if (!confirm("Ripristinare tutti i look di default? Le tue personalizzazioni saranno perse.")) return;
      Theme.set("chipStyles", {});
      render();
      toast("Look chip ripristinati", "success");
    });
  }
  ...
  render();
  // NIENTE Theme.subscribe(render): l'edit live del color picker chiama
  // Theme.set ad ogni keystroke -> re-render full + perdita focus.
  // render() viene chiamato manualmente solo dopo il reset globale.
}
